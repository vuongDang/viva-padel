...
export const AlarmService = {
  ...
  async syncAlarms(alarms, weeksAhead = 1) {
    ...
    const mappedAlarms = alarms.map((alarm) => {
      ...
      else if (!alarm.types.indoor && alarm.types.outdoor)
        court_type = "Outdoor";
  ...
  mapServerAlarmsToMobile(serverAlarms) {
    ...
    return serverAlarms.map((sa, index) => {
      ...
      return {
        ...
        weekdays: (sa.days_of_the_week || []).map(
          (day) => REVERSE_WEEKDAY_MAP[day],
        ),
        startTime: startTime,
        endTime: endTime,
        types: types,
