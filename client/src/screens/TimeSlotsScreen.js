  ...
  const getAvailableSlots = (dayPlanning, allowedDurations) => {
    ...
    dayPlanning.courts.forEach((court) => {
      ...
      court.slots.forEach((slot) => {
        ...
        if (
          slot.prices &&
          slot.prices.some((p) => p.bookable && durations.includes(p.duration))
        ) {
          slots.push({
            time: slot.start_at,
            court: court.name,
    ...
    <SafeAreaView style={styles.container}>
        ...
        {alarms.length === 0 ? (
          <View style={styles.emptyContent}>
            ...
            <Text style={styles.subtitle}>
              Configurez des alertes pour être notifié des disponibilités.
            </Text>
          </View>
        ) : (
                        ...
                        >
                          {matchingDays.map(([dateStr, dayAvail]) => (
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                handleDateClick(dateStr, dayAvail, alarm)
                              }
                            >
                              <Text
                                style={[styles.dayText, styles.localDayText]}
                              >
                                {formatDate(dateStr)}
                              </Text>
                            </TouchableOpacity>
                        ...
                        {matchingDays.length > 5 && (
                          ...
                          >
                            <Text style={styles.showMoreText}>
                              {isExpanded
                                ? "Voir moins"
                                : `Voir plus (${matchingDays.length - 5} de plus)`}
                            </Text>
                          </TouchableOpacity>
                        )}
                      </View>
                  ...
                  matchedResults[alarm.name] && (
                    <View style={styles.notifSection}>
                      <View style={styles.notifHeader}>
                        ...
                        <TouchableOpacity
                          onPress={() => onClearMatchedResult(alarm.name)}
                        >
                      ...
                      <View
                        style={[
                          styles.daysContainer,
                          !expandedAlarms.has(alarm.name) &&
                          Object.keys(matchedResults[alarm.name]).length >
                          5 &&
                          styles.collapsedContainer,
                        ]}
                      >
                        {Object.entries(matchedResults[alarm.name]).map(
                          ([date, dayPlan]) => (
                            <TouchableOpacity
                              key={date}
                              style={styles.dayChip}
                              onPress={() =>
                                handleDateClick(date, dayPlan, alarm)
                              }
                            >
                      ...
                      {Object.keys(matchedResults[alarm.name]).length > 5 && (
                        ...
                        >
                          <Text style={styles.showMoreText}>
                            {expandedAlarms.has(alarm.name)
                              ? "Voir moins"
                              : `Voir plus (${Object.keys(matchedResults[alarm.name]).length - 5} de plus)`}
                          </Text>
                        </TouchableOpacity>
                      )}
                    </View>
      ...
      {alarms.length > 0 && (
        <View style={styles.floatingButtonsContainer}>
          ...
          >
            <Text style={styles.notifFloatingButtonText}>
